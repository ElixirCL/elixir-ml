...
1. **Media Aritmética (stem:[\bar{x}])**: Representa el centro de gravedad de nuestros datos. Es el valor que esperaríamos encontrar bajo condiciones de perfecta uniformidad, aunque debemos tener cuidado, pues es sensible a valores extremos que pueden distorsionar la imagen de la realidad.
[stem]
--
...
3. **Moda**: Es el valor que aparece con mayor frecuencia en nuestra muestra. Un conjunto de datos puede ser unimodal (una sola moda), bimodal o multimodal.

4. **Media Ponderada**: Es necesaria cuando, por la naturaleza de nuestro experimento, decidimos otorgar distinta relevancia o "peso" a cada una de nuestras observaciones.
[stem]
--
...
=== Varianza (stem:[\sigma^2]) y Desviación Estándar (stem:[\sigma])
...
[NOTE]
====
Cuando trabajamos con una **muestra** en vez de una población completa, se utiliza la **cuasivarianza** stem:[S^2], dividiendo por stem:[n-1] en vez de stem:[n]. Esta corrección (llamada corrección de Bessel) compensa el sesgo de estimar la dispersión poblacional a partir de datos limitados.
====
...
[TIP]
====
Es una regla de observación muy útil considerar como "anómalo" (*outlier*) a cualquier valor que se sitúe fuera de estos límites: stem:[[Q_1 - 1.5 \cdot IQR, \; Q_3 + 1.5 \cdot IQR]]. No es un capricho, sino una forma lógica de discernir el orden.
====
...
=== Coeficiente de Asimetría de Fisher (stem:[G_1])
...
--
*   **stem:[G_1 = 0]**: Una estructura perfectamente equilibrada y simétrica.
*   **stem:[G_1 > 0]**: Los datos muestran una "cola" que se estira hacia la derecha.
*   **stem:[G_1 < 0]**: La inclinación ocurre hacia la izquierda.
...
*   **stem:[K = 3]** (o exceso stem:[= 0]): **Mesocúrtica**. Colas similares a la distribución Normal.
*   **stem:[K > 3]**: **Leptocúrtica**. Colas pesadas, mayor probabilidad de valores extremos. Frecuente en datos financieros.
*   **stem:[K < 3]**: **Platicúrtica**. Colas ligeras, los datos se concentran cerca del centro.
...
Cuando observamos dos variables (stem:[X] e stem:[Y]) al mismo tiempo, buscamos comprender la afinidad o dependencia que existe entre ellas.
...
2. **Coeficiente de Correlación de Pearson (stem:[r])**: Es una escala preciosa que va del stem:[-1] al stem:[1], permitiéndonos ver qué tan fuerte es el vínculo lineal entre nuestras dos magnitudes estudiadas.
[stem]
--
...
| Varianza
| stem:[\sigma^2 = \frac{1}{n}\sum(x_i - \bar{x})^2]
| `Nx.variance/1`

| Desviación Estándar
| stem:[\sigma = \sqrt{\sigma^2}]
